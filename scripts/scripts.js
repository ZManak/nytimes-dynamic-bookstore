...
async function getCategories() {
  let resp = await fetch(
    "https://api.nytimes.com/svc/books/v3/lists/names.json?api-key=GAcpHtEABQGQmAlDrXlHGWSjtTBLAo3A"
  );
...
async function getBooks(catName) {
  ...
  let resp = await fetch(
    `https://api.nytimes.com/svc/books/v3/lists/current/${catName}.json?api-key=GAcpHtEABQGQmAlDrXlHGWSjtTBLAo3A`
  );
...
function printCategories(categories, canvas) {
  ...
  for (let i = 0; i < arrayCategories.length; i++) {
    ...
    card.innerHTML = `<h3>${bookCategories[i].display_name}</h3>
...
function printBooks(books, canvas) {
  ...
  for (let i = 0; i < arrayBooks.length; i++) {
    ...
    card.innerHTML = `<h3>#${arrayBooks[i].rank} ${arrayBooks[i].title}</h3>
...
function removeFav(userID, bookObject) {
  getUserById(userID).then((user) => {
    user.ref.update({
      favs: firebase.firestore.FieldValue.arrayRemove(bookObject),
    });
...
firebase.auth().onAuthStateChanged(function (user) {
  if (user) {
    ...
    getAvatar(firebase.auth().currentUser.uid).then((url) => {
      if (url === undefined) {
        getElementById("avatar").setAttribute(
          "src",
          "assets/logos/defaultAvatar.png"
        );
      } else {
        printAvatar(url);
      }
